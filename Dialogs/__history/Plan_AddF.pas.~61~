...
begin
  ...
  begin
    ShowMessage('Ошибка обработки данных' + #13 + E.Message);
    Table1.Close;
  end;
  end;
end;
...
begin
 // label5.Caption:= inttostr(vid);
  if vid = 1 then                // редактирование не пустой записи
  begin
    with Query1 do
    begin
      Close;
      ...
      Open;
      if not IsEmpty then        // проверяем есть ли записи
      begin
  ...
  if vid = 2 then             // редактирование пустой записи
  begin
    JvDateedit1.date := dat1;
    TE_S.Time := tm1;
    TE_D.Time := tm2 - tm1;
  ...
  begin
    ...
    begin
      ...
      Jce.Text := FieldByName('naim').ASString;
      //JceM.Text := FieldByName('naimm').ASString;
      Edit1.Text := 'День рожденья. Тел: ' + FieldByName('tel').ASString;
      code_pr :=  id;
      Close;
    end;
    jce.Enabled := false;
