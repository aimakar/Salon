...
begin
  try
    ...
    NewProiz.Caption := 'ѕросмотр и редактирование сведений о производителе';
    DataMain.Proiz.Edit;
...
begin
  if MessageDlg('”далить запись?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin
    S := False;
    Query1.Close;
   ...
   {I2 := dataMain.ProizCODE_Proiz.AsInteger;
    ...
    begin
      ...
      ShowMessage('ѕо данному производителю было движение, удаление невозможно!');
  end;
...
begin
  try
    Application.CreateForm(TNewProiz, NewProiz);
    NewProiz.Caption := 'ƒобавление нового производител€';
    ...
    begin
      ...
      begin
        ...
        Predpr.Post;
        if MessageDlg('∆елаете отредактировать данные клиента?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
        begin
          try
