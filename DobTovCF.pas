...
case Key of
  ...
  AddKol.Label5.Caption := 'Р.З.Т.: '+DataMain.TovarRZT.AsString;
  // проверка есть ли в теле накладной выбранный товар
  L := Datamain.CashTn.Locate('Code;Nomer',VarArrayOf([I,DataMain.CashNaklNomer.AsString]),[]);

  if not(L) then    // товара нет в накладной
   begin
    AddKol.Up.Text := '0';
    AddKol.Kol.Text := '0';
   end
  else        // товар еcть в накладной
   begin
         ...
         begin
           ...
           With DataMain.Ost do
             begin
              ...
              then ShowMessage('Опоздали')
              else
                begin
                 Edit;
                 if (trim(DataMain.Tovar.FieldByName('Vid').AsString)='0') then  //если это товар
